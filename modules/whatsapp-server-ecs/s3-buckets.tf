...
resource "aws_s3_bucket" "pipeline_artifacts" {
  bucket        = "${local.app_name}-artifacts" # Update with your desired S3 bucket name
  force_destroy = true
}

# Bucket para las sesiones de WhatsApp (whatsapp-web.js)
resource "aws_s3_bucket" "whatsapp_sessions" {
  bucket        = "${local.app_name}-unique-tf" # Update with your desired S3 bucket name
  force_destroy = true
}
...
resource "aws_s3_object" "whatsapp_sessions_folder" {
  ...
  key    = "${local.app_name}-sessions-folder/"
